...
void runLaunchDaemons(void){
    ...
    int douicache = 0;
    // Bearded old boostrap
    if(![[NSFileManager defaultManager]fileExistsAtPath:@"/Applications/Zebra.app/"]){
        NSURL *bootstrapURL = [[NSBundle mainBundle]URLForResource:@"Bootstrap" withExtension:@"tar"];
        postProgress(@"installing Zebra");
        //NSLog(@"Didn't find Zebra.app (so we'll assume bearded old bootstrap isn't extracted, we will extract it)\n");
        NSLog(@"Extracting Zebra...\n");
        ...
        r = easyPosixSpawn([NSURL fileURLWithPath:@"/prep_bootstrap.sh"], @[@""]);
        if(r != 0){
            NSLog(@"posix_spawn returned nonzero value: %d, errno: %d, strerror: %s\n", r, errno, strerror(errno));
            return;
        }
